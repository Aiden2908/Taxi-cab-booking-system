<?php
    ...
    $conn=get_db_conn();
    if($conn!=null){
        $sql="SELECT * FROM cabRequest WHERE HOUR(TIMEDIFF(NOW(), timestamp))<=2";
        $result = mysqli_query($conn, $sql);

        if (mysqli_num_rows($result) > 0) {//Grabing data from db.
            echo "<h7 style='margin-bottom:20px' class='input100'>Booking requests within 2 hours</h7>";
            ...
            echo '<div class="input100">0 Booking requests within 2 hours of time found.</div>';
        }
